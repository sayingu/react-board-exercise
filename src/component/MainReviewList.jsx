...
const MainReviewList = () => {
    ...
    const getList = () => {
        setIsLoading(true);
        fetch(`${process.env.REACT_APP_API_URL}/lush/mainReview?page=${page}&countPerPage=${countPerPage}&srchName=${srchName}&srchWriter=${srchWriter}&srchDateFrom=${srchDateFrom}&srchDateTo=${srchDateTo}`).then(res => {
            setIsLoading(false);
            res.json().then(json => {
    ...
    return (
        <>
            ...
            <div className="field is-grouped is-grouped-right">
                <p className="control">
                    <button className="button is-primary" onClick={() => { navigate('/mainReview'); }}>등록</button>
                </p>
            </div>
            <table className="table is-bordered is-hoverable is-fullwidth" style={{ tableLayout: 'fixed' }}>
                ...
                <tbody>
                    {list && list.length > 0 ? list.map(value => (
                        <tr key={value.id} className="is-clickable" onClick={() => { navigate(`/mainReview/${value.id}`); }}>
                            <td>{value.no}</td>
                            <td>{value.name}</td>
