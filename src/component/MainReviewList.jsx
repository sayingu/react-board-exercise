...
const MainReviewList = () => {
    ...
    const getList = () => {
        setIsLoading(true);
        fetch(
            `/api/lush/mainReview?page=${page}&countPerPage=${countPerPage}&srchName=${srchName}&srchWriter=${srchWriter}&srchDateFrom=${srchDateFrom}&srchDateTo=${srchDateTo}`
        ).then((res) => {
            setIsLoading(false);
            if (res.status === 200) {
                res.json().then((json) => {
